...
func CreateZapFactory(entry func(zapcore.Entry) error) *zap.Logger {
	...
	// 获取程序所处的模式：  开发调试 、 生产
	appDebug := variable.ConfigYml.GetString("AppEnv")
	...
	encoderConfig.TimeKey = "created_time" // 生成json格式日志的时间键字段，默认为 ts,修改以后方便日志导入到 ELK 服务器
	...
	//写入器
	fileName := variable.BasePath + variable.ConfigYml.GetString("Logs.SkeletonLogName")
